...
const mongobootstrap = require( '../../src/dao/bootstrapdata' );
const udao = require( '../../src/dao/userprofile-dao' );
const apiRouter = require('../../src/middleware/api-routes');
...
const PORT = process.env.PRF_PORT || 3000;

//===== router declarations / imports
//var svrSocket = require( '../../src/middleware/socket-services' ); //for later

//======= Express App config.
app.use(bodyParser.urlencoded({ extended: true })); //everyting incuding the payload in POSTS will be urlencoded for simplicity.
app.use(bodyParser.json()); //get middleware that parses json http bodys, for use by our express app
...
//======= STARTUP!!
w.info("profilic::Server %%%%%%%%%% PROFILIC visual test harness APP | STARTING %%%%%%%%%");

//======== get a db connection and set up the server:
mongobootstrap.dbsetup( function(err) {
  if(err) {
    w.error("profilic::Server Can't start server due to mongodb connection error");
