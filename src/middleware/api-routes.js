...
function updateMultiple(req,res,next) {
    ...
    //let idList = profileObjList.map(p => p._id);
    let theNames = profileObjList.map(p => p.username);
    let updatePromises;
    ...
    Promise.all(updatePromises).then( (results)=> {
        //fetch back the items and send back:
        // w.debug("profilic::apiRouter::updateMultiple : rtng the updated objs via userDAO::findIn ");
        // UserProfileDAO.findIn("username", theNames, function(err, response) {
        //     if(err) {
        //         res.status(500).json({ profileList:[], message: 'An error occurred'});
        //     } else {
        //         res.json({ profileList:response, message: 'OK'});
...
function createLinkage(req,res,next) {
    ...
    try {
        ...
        w.warn("profilic::apiRouter::createLinkage 1");
        let updateList = [];
        ...
        w.warn("profilic::apiRouter::createLinkage 2");
        ...
        if(updateList.length > 0) {
            w.warn("profilic::apiRouter::createLinkage 3");
        ...
        console.log("profilic::apiRouter::createLinkage - an error occured");
        console.log(err);
        return next(err);
    }
}
